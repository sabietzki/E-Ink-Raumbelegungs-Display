...
if [ -f "data/no_wifi.bmp" ] || [ -f "data/low_battery.bmp" ] || [ -f "data/no_connection.bmp" ]; then
  ...
  [ "$icons_needed" = 1 ] && python3 embed_icons.py 2>/dev/null || true
fi

# Inter-Schriften: .h-Dateien aus TTF erzeugen, falls fehlend
if [ ! -f "Inter_extrabold_27pt7b.h" ] && [ ! -f "Inter_semibold_9pt7b.h" ]; then
  if [ -f "data/Inter_ExtraBold.ttf" ] || [ -f "data/Inter_SemiBold.ttf" ] || [ -f "data/Inter_bold.ttf" ]; then
    echo "Inter-Schriften werden erzeugt (ttf_to_gfx.py)..."
...
echo "Fertig. Dateien in $BINARIES_DIR: bootloader.bin, partitions.bin, app.bin"
